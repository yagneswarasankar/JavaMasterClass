...
import chapters.chapter4.ExpStatementMethods.PlayingCat.PlayingCat;
...
public class Main {
 ...
 /*   private static void removeSong(LinkedList<Song> playList){
        if(playList.size() ==0){
            System.out.println("There are no songs in the Play list ");

        }else{
            ListIterator<Song> iterator = playList.listIterator();
            if(iterator.hasNext()){
                System.out.println("Removing the song "+ iterator.remove());
                iterator.next();
                }else{
                iterator.remove();
                iterator.previous();
